...
const AddAppointment = () => {
  ...
  const getAppointeePackage = useCallback(
    async (id) => {
      setValue(`package`, '')
      setTimeSlotCount(4)
      try {
        var res = await axios.post('getByAppointee/package/', { id: id })
        let response = res?.data?.data
        if (res?.data?.status === 200) {
          appointeeDispatch({
            type: 'PACKAGE',
  ...
  const AppointeeChange = useCallback(
    async (id) => {
      ...
      try {
        let res = await axios.post('/getSlotByAppointee/slot', { user_id: id })
        let response = res?.data?.data
        if (res?.data?.status === 200) {
          slotsDispatch({
        ...
        let bookedSlotsRes = await axios.post('/getPendingApprovedByDate/appointment', {
          appointeeId: id,
          status: [0, 1],
          appointmentDate: moment(startSlotDate).format('YYYY-MM-DD'),
        })
  ...
  }, [appointeeDispatch, getAllAppointee])
  ...
  useLayoutEffect(() => {
    ...
    if (slots?.appointeeSlots?.length > 0 && startSlotDate) {
      for (var i = 0; i < 7; i++) {
        ...
        arr.push({ date: date, day: findSlot?.day, timeslot: chunkArr })
      }
      setDaySlotArr(arr)
    }
  ...
  const onSubmit = async (data) => {
    if (formData?.slot?.date && formData?.slot?.time) {
      ...
      try {
        var res = await axios.post('/add/appointment', request)
        if (res?.data?.status === 200) {
          ...
          <CCardBody>
            <form onSubmit={handleSubmit(onSubmit)}>
              <FormControl fullWidth>
                <InputLabel id="demo-multiple-name-label">Appointee</InputLabel>
                <Select
                  name="appointee"
                  variant="standard"
                  fullWidth
                  ...
                  MenuProps={MenuProps}
                  onChange={(e) => {
                    setValue(`appointee`, e?.target?.value, {
                      shouldValidate: true,
                      shouldDirty: true,
                    })
                    AppointeeChange(e?.target?.value)
                  }}
                  value={formData?.appointee ? formData?.appointee : ''}
                  inputRef={register(`appointee`, {
                    required: 'Please select appointee',
                  })}
                >
                  {appointee?.appointeelist?.map((appoint) => (
                    <MenuItem key={appoint?._id} value={appoint?._id}>
                      {appoint?.name}
                    </MenuItem>
                  ))}
                </Select>
                <p className="error font15">
                  {errors && errors['appointee'] && errors['appointee']?.message}
                </p>
              </FormControl>
              <FormControl fullWidth style={{ marginTop: 20 }}>
                <InputLabel id="demo-multiple-name-label">Package</InputLabel>
                <Select
                  name="package"
                  variant="standard"
                  fullWidth
                  ...
                  inputRef={register(`package`, {
                    required: 'Please select package',
                  })}
                >
                  {appointee?.packagelist?.map((pack) => (
                ...
                <p className="error font15">
                  {errors && errors['package'] && errors['package']?.message}
                </p>
              </FormControl>
              <FormControl fullWidth style={{ marginTop: 20 }}>
                <TextareaAutosize
